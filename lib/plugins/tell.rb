...
module Plugins
  class Tell
    ...
    def tell(m, nick, msg)
      ...
      $redis.sadd "remind:#{@bot.irc.network.name.downcase}", nick.irc_downcase(:rfc1459)
      $redis.rpush "remind:#{@bot.irc.network.name.downcase}:#{nick.irc_downcase(:rfc1459)}", data.to_json
    ...
    def get_unread(nick)
      if $redis.sismember "remind:#{@bot.irc.network.name.downcase}", nick.irc_downcase(:rfc1459)
        key = "remind:#{@bot.irc.network.name.downcase}:#{nick.irc_downcase(:rfc1459)}"
        unread = $redis.lrange key, 0, -1
        $redis.del key
        $redis.srem "remind:#{@bot.irc.network.name.downcase}", nick.irc_downcase(:rfc1459)
